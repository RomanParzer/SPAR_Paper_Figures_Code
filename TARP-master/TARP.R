...
rRM.MBCR=function(k,psi)
{
	p1=psi;	p2=(1-2*psi); p3=psi
  	...
  	IND3=which(U>(p1+p2))
  	GRV[IND1]=-1/sqrt(2*psi)
	GRV[IND2]=0
  	GRV[IND3]=1/sqrt(2*psi)
  	return(GRV)
}
...
# Y_hat 
    ...
    # add beta 
    betas[I_use,i] <- RM.star%*%solve.SG%*%beta_hat1
...
# paramterters and DF of t-distribution
  ...
  # return(Res.MBCR)
  return(list(res_matrix=Res.MBCR,beta=rowMeans(betas)))
}
