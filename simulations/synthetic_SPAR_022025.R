...
methods <- list("HOLP"=myOLS,
                ...
                "BAMP true sparsity" = function(x,y,xtest,gamma){myBAMP(x,y,xtest,gamma=gamma)})
...
foreach(i=1:nrep) %dopar% {
    ...
    for (k in 1:nmethods) {
      ...
      tstamp <- Sys.time()
      if (k==nmethods) {
        callres <- tryCatch( methods[[k]](x,y,xtest,gamma=(p-a)/p),
...
saveRDS(list(res=res,fits=fits),
        file = sprintf("../saved_results/synth_rateye_SPAR_25_nset%d_reps%d_nmeth%d.rds",nset,nrep,nmethods))
