...
# rMSPE <- array(NA,dim=c(nrep,2,length(nscreens)),dimnames = list(reps=1:nrep,methods=c("Screening","ScrRP"),nscreens=nscreens))
# 
# for (i in 1:nrep) {
...
#                              })
#       } else {
#         solve_tmp <- tryCatch( solve(tcrossprod(zz),yz),
#                                error=function(error_message) {
#                                  return(solve(tcrossprod(zz)+0.01*diag(n),yz))
#                                })
...
# }
# 
# saveRDS(rMSPE,"./Result_ScrRP_nscreen_no1.rds")

rMSPE <- readRDS("Result_ScrRP_nscreen_no1.rds")
nrep <- dim(rMSPE)[1]
...
levels(sum_df_rMSPE$Method) <- c("Scr_HOLP","ScrRP")
...
sum_df_rMSPE %>%
  ...
  geom_text(data=data.frame(x= 1,
            ...
            aes(x=x,y=y,label=label),col=1,nudge_y = 15) +
  geom_text(data=data.frame(x= 1,
                            y = minScr,
                            Method="ScrRP",
                            label=round(minScr,1)),
            aes(x=x,y=y,label=label),col=1,nudge_y = 0.2) +
  scale_x_log10() +
