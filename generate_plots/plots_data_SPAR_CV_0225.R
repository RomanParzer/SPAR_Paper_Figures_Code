...
resobj <- readRDS("../saved_results/data_SPAR_25_nset3_reps100_nmeth20.rds")
...
mydf_all %>% filter(Method %in% show_methods,dataset=="face",) %>%
  ...
  # geom_segment(aes(x = c("OWL"), y = c(0.025), xend = c("OWL"), yend = c(0.03)),
  #              arrow = arrow(length = unit(0.5, "cm")),col=scales::hue_pal()(10)[6],linetype=2) +
  ...
  # annotate("text",x="OWL",y=0.0225,label=paste0("median\n ",round(med_df$MedrMSPE[med_df$Method=="OWL"],3))) +
  coord_cartesian(ylim=c(0,0.03)) +
  theme(legend.position = "none")
# ggsave(paste0("../plots/SPAR_CV_rMSPE_face_25.pdf"), height = 4, width = 8)

# show_methods <- c("BAMP","LASSO",show_methods)
# ggsave(paste0("../plots/SPAR_CV_rMSPE_face_25_BAMP_LASSO.pdf"), height = 4, width = 8)
...
mydf_all %>% filter(Method %in% c(show_methods,"Ridge"),dataset!="face",Method!="PLS") %>%
  ...
  # coord_cartesian(ylim=c(0,10)) + for BAMP
  scale_linetype(guide="none") +
  geom_hline(yintercept=1,linetype=2)
# ggsave(paste0("../plots/SPAR_CV_rMSPE_rateye_25.pdf"), height = 4, width = 8)
# ggsave(paste0("../plots/SPAR_CV_rMSPE_rateye_25_BAMP_LASSO.pdf"), height = 4, width = 8)


med_df_all <- mydf_all %>% group_by(Method,dataset) %>% summarize(MedrMSPE=median(rMSPE,na.rm=TRUE),MednumAct=median(numAct,na.rm=TRUE))
